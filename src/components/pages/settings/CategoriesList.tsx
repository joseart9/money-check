"use client";

import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { deleteCategory, fetchCategories } from "./settings.api";

export function CategoriesList() {
  const qc = useQueryClient();
  const query = useQuery({
    queryKey: ["settings:categories"],
    queryFn: fetchCategories,
  });

  const del = useMutation({
    mutationFn: async (id: string) => deleteCategory(id),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["settings:categories"] });
    },
  });

  if (query.isLoading) {
    return <p className="text-base text-muted-foreground">Cargando...</p>;
  }

  if (query.isError) {
    return (
      <p className="text-base text-red-500">
        No se pudieron cargar las categorías.
      </p>
    );
  }

  if (query.data?.length === 0) {
    return (
      <p className="text-base text-muted-foreground">
        Aún no tienes categorías registradas.
      </p>
    );
  }

  return (
    <div className="space-y-3 h-full overflow-y-auto">
      {query.data?.map((c) => {
        const chipColor =
          c.type === "INCOME" ? "bg-green-500/15 text-green-700" : "bg-red-500/15 text-red-700";
        const chipLabel = c.type === "INCOME" ? "Ingreso" : "Gasto";

        return (
          <div
            key={c.id}
            className="flex items-center justify-between gap-3 rounded-2xl p-4 shadow-sm bg-accent/50 dark:bg-accent/50"
          >
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <p className="truncate text-base text-accent-foreground font-medium">{c.name}</p>
                <span
                  className={`shrink-0 rounded-full px-4 py-2 text-sm font-medium leading-none ${chipColor}`}
                >
                  {chipLabel}
                </span>
              </div>
              {c.description && (
                <p className="mt-1 truncate text-base text-accent-foreground/50">
                  {c.description}
                </p>
              )}
            </div>

            <Button
              type="button"
              variant="ghost"
              size="icon-lg"
              className="rounded-2xl"
              onClick={() => del.mutate(c.id)}
              disabled={del.isPending}
              aria-label="Eliminar categoría"
            >
              <Trash2 className="h-5 w-5" />
            </Button>
          </div>
        );
      })}
    </div>
  );
}

